...
class SweIoosTest(unittest.TestCase):
    def test_o_and_m_get_observation(self):
        data = open(
            resource_file(os.path.join("ioos_swe", "OM-GetObservation.xml")),
            "rb",
        ).read()
        ...
        ts = d.observations[0]
        assert (
            ts.description.replace("\n", "")
            .replace("       ", "")
            .replace("  -", " -")
            == "Observations at point station urn:ioos:station:wmo:41001, 150 NM East of Cape HATTERAS. Observations at point station urn:ioos:station:wmo:41002, S HATTERAS - 250 NM East of Charleston, SC"  # noqa
        )
        assert ts.begin_position == datetime(2009, 5, 23, 0, tzinfo=pytz.utc)
        assert ts.end_position == datetime(2009, 5, 23, 2, tzinfo=pytz.utc)
        assert sorted(ts.procedures) == sorted(
            ["urn:ioos:station:wmo:41001", "urn:ioos:station:wmo:41002"]
        )
        assert sorted(ts.observedProperties) == sorted(
            [
        ...
        assert ts.location["urn:ioos:station:wmo:41001"].equals(
            Point(-72.73, 34.7)
        )
        assert ts.location["urn:ioos:station:wmo:41002"].equals(
            Point(-75.415, 32.382)
        )

    def test_timeseries_multi_station_multi_sensor(self):
        swe = open(
            resource_file("ioos_swe/SWE-MultiStation-TimeSeries.xml"), "rb"
        ).read()
    ...
    def test_timeseries_single_station_single_sensor(self):
        ...
        assert sorted(
            (x.time.strftime("%Y-%m-%dT%H:%M:%SZ") for x in station.elements)
        ) == sorted(
            [
        ...
        assert sorted((x["value"] for x in first_members)) == sorted(
            [2.0, 15.4, 280]
        )
        assert sorted((x["standard"] for x in first_members)) == sorted(
            [
                "http://mmisw.org/ont/cf/parameter/air_temperature",
    ...
    def test_timeseries_profile_single_station(self):
        swe = open(
            resource_file(
                "ioos_swe/SWE-SingleStation-TimeSeriesProfile_QC.xml"
            ),
            "rb",
        ).read()
        data_record = etree.fromstring(swe)
        ...
        assert (
            time_range[0].strftime("%Y-%m-%dT%H:%M:%SZ")
            == "2009-05-23T00:00:00Z"
        )
        assert (
            time_range[-1].strftime("%Y-%m-%dT%H:%M:%SZ")
            == "2009-05-23T02:00:00Z"
        )
        ...
        assert (
            profile.time.strftime("%Y-%m-%dT%H:%M:%SZ")
            == "2009-05-23T00:00:00Z"
        )
        assert len(profile.elements) == 4
        assert [e.location.z for e in profile.elements] == [
            -39.5,
            -19.5,
            -9.5,
            -4.5,
        ...
        assert [m["value"] for m in profile.elements[0].members] == [
            352.0,
            9.6,
        ]
        ...
        assert (
            sensor["sensor_orientation"]["X"]["name"] == "platform_pitch_angle"
        )
        assert (
            sensor["sensor_orientation"]["Y"]["name"] == "platform_roll_angle"
        )
        assert (
            sensor["sensor_orientation"]["Z"]["name"] == "platform_orientation"
        )
