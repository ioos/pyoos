...
class IoosDescribeSensorTest(unittest.TestCase):
    def test_network(self):
        data = open(
            resource_file(
                os.path.join("ioos_swe", "SML-DescribeSensor-Network.xml")
            ),
            "rb",
        ).read()
        ...
        assert d.procedures == sorted(
            [u"urn:ioos:station:wmo:41001", u"urn:ioos:station:wmo:41002"]
        )
    ...
    def test_station(self):
        data = open(
            resource_file(
                os.path.join("ioos_swe", "SML-DescribeSensor-Station.xml")
            ),
            "rb",
        ).read()
        d = IoosDescribeSensor(data)
        ...
        assert d.variables == sorted(
            [
                u"http://mmisw.org/ont/cf/parameter/sea_water_temperature",
    ...
    def test_sensor(self):
        data = open(
            resource_file(
                os.path.join("ioos_swe", "SML-DescribeSensor-Sensor.xml")
            ),
            "rb",
        ).read()
        d = IoosDescribeSensor(data)
        ...
        assert (
            d.system.name
            == "urn:ioos:sensor:us.glos:45023:sea_water_temperature"
        )
