...
class CollectorTest(unittest.TestCase):
    def test_times_return_utc(self):
        ...
        assert c._start_time == datetime(
            2000, 2, 1, 10, tzinfo=timezone("UTC")
        )
        assert c._end_time == datetime(2000, 2, 3, 8, tzinfo=timezone("UTC"))

        # Always returns UTC (5 hour difference here)
        c.filter(
            start=datetime(2000, 2, 1, 10, tzinfo=Eastern),
            end=datetime(2000, 2, 3, 8, tzinfo=Eastern),
        )
        assert c._start_time == datetime(
            2000, 2, 1, 15, tzinfo=timezone("UTC")
        )
