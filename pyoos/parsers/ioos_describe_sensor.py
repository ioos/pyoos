...
from owslib.util import nspath_eval as nsp
from owslib.util import testXMLValue, testXMLAttribute, extract_xml_list
...
class IoosDescribeSensor(object):
    def __init__(self, element):
        ...
        # TODO: Are we ever going to have more than one sml:member?
        self.systems = []
...
class SystemMetadata(object):
    ...
    def __init__(self, element):
        ...
        for event_member in self._root.findall(nsp('sml:history/sml:EventList/sml:member')):
            events = EventMemberMetadata(event_member)
            self.history[events.name] = events
...
class EventMemberMetadata(object):
    ...
    def __init__(self, element):
        ...
        for event in self._root.findall(nsp('sml:Event')):
            self.events.append(EventMetadata(event))
