...
class CoopsSos(IoosSweSos):
    ...
    def list_datums(self):
        # TODO: Get from GetCaps
        return ["MLLW", "MSL", "MHW", "STND", "IGLD", "NAVD"]
    ...
    def setup_params(self, **kwargs):
        ...
        if self.features is None or len(self.features) != 1:
            ...
            params["offerings"] = [
                "urn:ioos:station:NOAA.NOS.CO-OPS:%s" % self.features[0]
            ]
        ...
        if self.datum is not None:
            if self.datum in self.list_datums():
                if self.datum == "NAVD":
                    ...
                    params["result"] = (
                        "VerticalDatum==urn:ioos:def:datum:noaa::%s"
                        % self.datum
                    )
            else:
                raise ValueError(
                    "Not a supported datum.  Use list_datums() for options"
                )
        ...
        if self.dataType is not None:
            if self.dataType in self.list_datatypes():
                ...
                raise ValueError(
                    "Not a supported data type.  Use list_datatypes() for options"
                )
