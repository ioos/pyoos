...
class WqpRest(Collector):
    def __init__(self, **kwargs):
        ...
        self.sites_url = kwargs.get(
            "sites_url", "http://www.waterqualitydata.us/Station/search"
        )
        self.results_url = kwargs.get(
            "results_url", "http://www.waterqualitydata.us/Result/search"
        )
        self.characteristics_url = kwargs.get(
            "characteristics_url",
            "http://www.waterqualitydata.us/Codes/Characteristicname",
        )
        self.characteristic_types_url = kwargs.get(
            "characteristic_types_url",
            "http://www.waterqualitydata.us/Codes/Characteristictype",
        )
    ...
    def set_features(self, features):
        ...
        if self._features is not None and len(self._features) > 1:
            raise ValueError(
                "WQP only supports extracting one feature at a time \
    ...
    def list_features(self):
        raise NotImplementedError(
            "WQP does not allow returning a list of unique features."
        )
